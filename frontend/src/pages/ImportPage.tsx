...
import type { ImportPreviewResponse, ImportResult, ExecuteRow, ImportRow } from "@/types";
    ...
    rows.map((row) => {
      ...
      return (
        <React.Fragment key={row.row_number}>
                      ...
                      {isMatched && !isOverride ? (
                        ...
                        <>
                          <span className="text-amber-600">
                            {p.name} <span className="font-medium">[新規]</span>
                          </span>
                          <button
                            type="button"
                            className="text-[10px] text-muted-foreground underline hover:text-foreground border-l border-border pl-1.5"
                        ...
                        <span>
                          {p.name} <span className="text-muted-foreground">[新規]</span>
                        </span>
                      )}
                    </div>
                  );
          ...
          {(row.directory_path || (showErrors && row.errors && row.errors.length > 0)) && (
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-sm">
              全 {preview.rows.length} 行 / インポート対象 {importCount} 件 / エラー{" "}
              {preview.error_count} 件
            </span>
            <div className="flex gap-2 ml-auto flex-wrap">
              <Button variant="outline" size="sm" onClick={selectAll}>全選択</Button>
              <Button variant="outline" size="sm" onClick={deselectAll}>全解除</Button>
              <Button variant="outline" onClick={reset}>キャンセル</Button>
              <Button onClick={() => setPhase("confirm")} disabled={importCount === 0}>
                確認へ →（{importCount}件）
              </Button>
            </div>
          </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <div className="flex gap-2 ml-auto">
              <Button variant="outline" onClick={() => setPhase("preview")}>← 戻る</Button>
          ...
          <div className="rounded-lg border overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                {renderRows(
                  preview.rows.filter((r) => r.is_valid && !rowStates[r.row_number]?.skipped),
                  false
                )}
              </tbody>
            </table>
